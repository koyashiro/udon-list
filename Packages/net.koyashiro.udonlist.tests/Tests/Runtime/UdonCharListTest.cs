...
namespace Koyashiro.UdonList.Tests
{
    ...
    public class UdonCharListTest : UdonSharpBehaviour
    {
        private void Start()
        {
            var list = UdonList<char>.New();
            list.Add('a');
            list.Add('b');
            list.Add('c');
            Assert.Equal('a', list.GetValue(0), this);
            Assert.Equal('b', list.GetValue(1), this);
            Assert.Equal('c', list.GetValue(2), this);
            Assert.Equal(new char[] { 'a', 'b', 'c' }, list.ToArray(), this);
            Assert.True(list.Remove('b'), this);
            Assert.False(list.Remove('d'), this);
            Assert.Equal('a', list.GetValue(0), this);
            Assert.Equal('c', list.GetValue(1), this);
            Assert.Equal(new char[] { 'a', 'c' }, list.ToArray(), this);
            list.SetValue(1, 'b');
            Assert.Equal(new char[] { 'a', 'b' }, list.ToArray(), this);

            list = UdonList<char>.New(new char[] { '0', '1', '2', '3', '4' });
            list.Reverse();
